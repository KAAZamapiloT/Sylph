...
GalleryExample::GalleryExample(int width, int height)
    : shader_(load_shader_source("shaders/glossy.vert"), load_shader_source("shaders/glossy.frag")),
      lighting_(8)
{
    std::cout << "Loading modular room and display meshes...\n";
    
    // 1. Helper function to load a mesh into a shared_ptr
    ...
    bunny.transform.set_position(Eigen::Vector3f(-1.5f, 0.0f, 0.0f));
    ...
    teapot.transform.set_position(Eigen::Vector3f(0.0f, 0.0f, 0.0f));
    teapot.roughness = 40.0f;
    objects_.push_back(std::move(teapot));
    
    // Dragon (Highly Polished)
    SceneObject dragon;
    dragon.mesh = mesh_dragon;
    dragon.transform.set_position(Eigen::Vector3f(1.5f, 0.0f, 0.0f));
    dragon.roughness = 150.0f;
    objects_.push_back(std::move(dragon));
    
    camera_.set_position(Eigen::Vector3f(0.0f, 0.0f, 4.0f));
...
void GalleryExample::update_grid_data()
{
    /*
...
void GalleryExample::update_roughness()
{
    sylph::SphericalHarmonics sh;
...
void GalleryExample::process_event(const SDL_Event& event, SDL_Window* window)
{
...
void GalleryExample::generate_lighting(float time)
{
    lighting_.clear();
...
void GalleryExample::update(float dt)
{
...
void GalleryExample::resize(int width, int height)
{
    if (height > 0)
        camera_.set_aspect_ratio(static_cast<float>(width) / height);
}

void GalleryExample::render(render::Renderer&)
{
    shader_.bind();
